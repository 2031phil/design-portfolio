...
closeButton.addEventListener("mouseover", function () {
    closeButton.style.transform = "translateY(-2.8rem) translateX(-.3rem) scale(1.1)";
});

closeButton.addEventListener("mouseleave", function () {
    closeButton.style.transform = "translateY(-2.8rem) translateX(-.3rem) scale(1)";
});

window.addEventListener("DOMContentLoaded", () => {
    if (window.innerWidth < 700) {
        setTimeout(function () {
            projectContainer.style.transition = ".5s cubic-bezier(.52, 0, .83, .64)";
        }, 50);
    }
});
